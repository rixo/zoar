...
const forkRunAction = ({ files }, options) => {
  let cancelLast
  ...
  return () =>
    new Promise((resolve, reject) => {
      ...
      const modulePath = path.resolve(__dirname, 'run.js')
      const args = []
      const execArgv = []
      const env = {}
...
export const watchAction = async ({ watch, files }, options) => {
  const { print, watchDebounce = 20, watchFilenames = false } = options
  ...
  const doRun = print
    ? async () => {
        ...
        return listAction({ files, footer: '\n' })
      }
    : forkRunAction({ files }, options)
