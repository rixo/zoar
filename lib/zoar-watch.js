...
import { fileMatcher, nope, joinGlob, toPosixSlashes, UserError } from './util'
import Debug from './debug'
import { actionRunner } from './action'
import Log from './util/log'
import interactive from './watch-interactive'
...
export const watchAction = async ({ watch, files }, initialOptions) => {
  let options = { ...initialOptions }

  const { watchDebounce = 20, watchFilenames, dump: isDump } = options

  let timeout = null

  let doRun

  const setOptions = (opts, andRun = true) => {
  ...
  // interactive cli
  interactive({ setOptions, options, run, dump })
