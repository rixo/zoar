...
const boolHandler = ({
  ...
  short,
  long,
  options: { alias: extraAlias = [], opt = long } = {},
}) => {
...
const filterHandler = ({
  ...
  multiple,
  optional,
  options: { opt = long, monoArg = false, name = long || short } = {},
}) => ({
  test: includes(
    ...
    const getValue = () => {
      ...
      if (!multiple) {
        ...
        if (args.length === 0) {
          ...
          if (reset) return initialOptions[opt]
      ...
      if (args.length === 0) {
        ...
        if (reset) return initialOptions[opt]
        if (pop) {
          const items = defaultArray(options[opt]).slice(0, -1)
      ...
      let items = rm
        ? []
        : reset
        ? defaultArray(initialOptions[opt])
        : defaultArray(options[opt])
...
export default (specs, { switches = [], hooks: inputHooks = [] } = {}) => {
  ...
  const handler = _api => {
    const { setOptions: _setOptions, options } = _api
    ...
    return { handle, setOptions, reset }
  }
