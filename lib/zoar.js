...
Promise.resolve()
  // custom options & alias
  .then(async () => {
    // custom options
    const config = await parseConfig({})
    if (config && config.customOptions) {
      for (const spec of config.customOptions) {
        program.option(...spec)
      }
    }
    ...
    if (config && config.alias) {
      for (const [spec, aliased] of Object.entries(config.alias)) {
  ...
  // run
  .then(args => program.action(action).parseAsync(args))
  .catch(err => {
