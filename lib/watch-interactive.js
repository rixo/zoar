...
const autoPipes = (opt, { setOptions }) => {
  let pipesBefore = opt === 'run'
  // for reset
  setOptions({ [opt]: pipesBefore })
  return (opts, { pipes }) => {
    const { [opt]: value } = opts
    if (value) {
      // entering run
      opts.pipes = pipesBefore
    } else {
      // leaving
      pipesBefore = pipes
    }
...
const parser = interactiveParser(
  [
    ...
    ['--pipe <command...>', { monoArg: true }], // TODO should be <commands:pipe...>
    ['--pipes', { alias: 'pp' }],
