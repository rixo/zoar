...
const commands = ({ alias, customOptions }) => [
  '--json',
  '--tap',
  '--indent',
  ...
  ...(customOptions && customOptions.map(x => x[0])),

  ...(alias && aliasParsers(alias)),
...
export default _api => {
  const { initialOptions, run, options, dump, dumpInput } = _api
  ...
  const parser = interactiveParser(commands(options), {
    switches: [
      ['run', 'print', 'ls'],
      ['json', 'tap'],
      ['inspect', 'inspect-brk'],
